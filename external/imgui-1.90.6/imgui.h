...
namespace ImGui
{
    ...
    //  - without IMGUI_DISABLE_OBSOLETE_KEYIO (legacy support): you can still use your legacy native/user indices (< 512) according to how your backend/circle stored them in io.KeysDown[], but need to cast them to ImGuiKey.
...
enum ImGuiConfigFlags_
{
    ...
    // User storage (to allow your backend/circle to communicate to code that may be shared between multiple projects. Those flags are NOT used by core Dear ImGui)
...
struct ImGuiIO
{
    ...
    bool        KeysDown[ImGuiKey_COUNT];           // [LEGACY] Input: Keyboard keys that are pressed (ideally left in the "native" order your circle has access to keyboard keys, so you can use your own defines/enums for keys). This used to be [512] sized. It is now ImGuiKey_COUNT to allow legacy io.KeysDown[GetKeyIndex(...)] to work without an overflow.
...
// The code expect ImVec2 pos (8 bytes), ImVec2 uv (8 bytes), ImU32 col (4 bytes), but you can re-order them or add other fields as needed to simplify integration in your circle.
...
struct ImFontAtlas
{
    ...
    // User is in charge of copying the pixels into graphics memory (e.g. create a texture with your circle). Then store your texture handle with SetTexID().
